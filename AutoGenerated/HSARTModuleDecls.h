//==============================================================================
// Copyright (c) 2015-2016 Advanced Micro Devices, Inc. All rights reserved.
/// \author AMD Developer Tools Team
/// \file
/// \brief THIS CODE WAS AUTOGENERATED BY PASSTHROUGHGENERATOR ON 08/24/16
//==============================================================================

#ifndef _HSARTMODULEDECLS_H_
#define _HSARTMODULEDECLS_H_

#include <hsa.h>
#include <hsa_ext_finalize.h>
#include <hsa_ext_image.h>
#include <hsa_ext_amd.h>
#include <hsa_ven_amd_loader.h>

typedef decltype(hsa_status_string)* hsa_status_string_t;
typedef decltype(hsa_init)* hsa_init_t;
typedef decltype(hsa_shut_down)* hsa_shut_down_t;
typedef decltype(hsa_system_get_info)* hsa_system_get_info_t;
typedef decltype(hsa_extension_get_name)* hsa_extension_get_name_t;
typedef decltype(hsa_system_extension_supported)* hsa_system_extension_supported_t;
typedef decltype(hsa_system_major_extension_supported)* hsa_system_major_extension_supported_t;
typedef decltype(hsa_system_get_extension_table)* hsa_system_get_extension_table_t;
typedef decltype(hsa_system_get_major_extension_table)* hsa_system_get_major_extension_table_t;
typedef decltype(hsa_agent_get_info)* hsa_agent_get_info_t;
typedef decltype(hsa_iterate_agents)* hsa_iterate_agents_t;
typedef decltype(hsa_agent_get_exception_policies)* hsa_agent_get_exception_policies_t;
typedef decltype(hsa_cache_get_info)* hsa_cache_get_info_t;
typedef decltype(hsa_agent_iterate_caches)* hsa_agent_iterate_caches_t;
typedef decltype(hsa_agent_extension_supported)* hsa_agent_extension_supported_t;
typedef decltype(hsa_agent_major_extension_supported)* hsa_agent_major_extension_supported_t;
typedef decltype(hsa_signal_create)* hsa_signal_create_t;
typedef decltype(hsa_signal_destroy)* hsa_signal_destroy_t;
typedef decltype(hsa_signal_load_acquire)* hsa_signal_load_acquire_t;
typedef decltype(hsa_signal_load_scacquire)* hsa_signal_load_scacquire_t;
typedef decltype(hsa_signal_load_relaxed)* hsa_signal_load_relaxed_t;
typedef decltype(hsa_signal_store_relaxed)* hsa_signal_store_relaxed_t;
typedef decltype(hsa_signal_store_screlease)* hsa_signal_store_screlease_t;
typedef decltype(hsa_signal_store_release)* hsa_signal_store_release_t;
typedef decltype(hsa_signal_silent_store_relaxed)* hsa_signal_silent_store_relaxed_t;
typedef decltype(hsa_signal_silent_store_screlease)* hsa_signal_silent_store_screlease_t;
typedef decltype(hsa_signal_exchange_scacq_screl)* hsa_signal_exchange_scacq_screl_t;
typedef decltype(hsa_signal_exchange_acq_rel)* hsa_signal_exchange_acq_rel_t;
typedef decltype(hsa_signal_exchange_scacquire)* hsa_signal_exchange_scacquire_t;
typedef decltype(hsa_signal_exchange_acquire)* hsa_signal_exchange_acquire_t;
typedef decltype(hsa_signal_exchange_relaxed)* hsa_signal_exchange_relaxed_t;
typedef decltype(hsa_signal_exchange_screlease)* hsa_signal_exchange_screlease_t;
typedef decltype(hsa_signal_exchange_release)* hsa_signal_exchange_release_t;
typedef decltype(hsa_signal_cas_scacq_screl)* hsa_signal_cas_scacq_screl_t;
typedef decltype(hsa_signal_cas_acq_rel)* hsa_signal_cas_acq_rel_t;
typedef decltype(hsa_signal_cas_scacquire)* hsa_signal_cas_scacquire_t;
typedef decltype(hsa_signal_cas_acquire)* hsa_signal_cas_acquire_t;
typedef decltype(hsa_signal_cas_relaxed)* hsa_signal_cas_relaxed_t;
typedef decltype(hsa_signal_cas_screlease)* hsa_signal_cas_screlease_t;
typedef decltype(hsa_signal_cas_release)* hsa_signal_cas_release_t;
typedef decltype(hsa_signal_add_scacq_screl)* hsa_signal_add_scacq_screl_t;
typedef decltype(hsa_signal_add_acq_rel)* hsa_signal_add_acq_rel_t;
typedef decltype(hsa_signal_add_scacquire)* hsa_signal_add_scacquire_t;
typedef decltype(hsa_signal_add_acquire)* hsa_signal_add_acquire_t;
typedef decltype(hsa_signal_add_relaxed)* hsa_signal_add_relaxed_t;
typedef decltype(hsa_signal_add_screlease)* hsa_signal_add_screlease_t;
typedef decltype(hsa_signal_add_release)* hsa_signal_add_release_t;
typedef decltype(hsa_signal_subtract_scacq_screl)* hsa_signal_subtract_scacq_screl_t;
typedef decltype(hsa_signal_subtract_acq_rel)* hsa_signal_subtract_acq_rel_t;
typedef decltype(hsa_signal_subtract_scacquire)* hsa_signal_subtract_scacquire_t;
typedef decltype(hsa_signal_subtract_acquire)* hsa_signal_subtract_acquire_t;
typedef decltype(hsa_signal_subtract_relaxed)* hsa_signal_subtract_relaxed_t;
typedef decltype(hsa_signal_subtract_screlease)* hsa_signal_subtract_screlease_t;
typedef decltype(hsa_signal_subtract_release)* hsa_signal_subtract_release_t;
typedef decltype(hsa_signal_and_scacq_screl)* hsa_signal_and_scacq_screl_t;
typedef decltype(hsa_signal_and_acq_rel)* hsa_signal_and_acq_rel_t;
typedef decltype(hsa_signal_and_scacquire)* hsa_signal_and_scacquire_t;
typedef decltype(hsa_signal_and_acquire)* hsa_signal_and_acquire_t;
typedef decltype(hsa_signal_and_relaxed)* hsa_signal_and_relaxed_t;
typedef decltype(hsa_signal_and_screlease)* hsa_signal_and_screlease_t;
typedef decltype(hsa_signal_and_release)* hsa_signal_and_release_t;
typedef decltype(hsa_signal_or_scacq_screl)* hsa_signal_or_scacq_screl_t;
typedef decltype(hsa_signal_or_acq_rel)* hsa_signal_or_acq_rel_t;
typedef decltype(hsa_signal_or_scacquire)* hsa_signal_or_scacquire_t;
typedef decltype(hsa_signal_or_acquire)* hsa_signal_or_acquire_t;
typedef decltype(hsa_signal_or_relaxed)* hsa_signal_or_relaxed_t;
typedef decltype(hsa_signal_or_screlease)* hsa_signal_or_screlease_t;
typedef decltype(hsa_signal_or_release)* hsa_signal_or_release_t;
typedef decltype(hsa_signal_xor_scacq_screl)* hsa_signal_xor_scacq_screl_t;
typedef decltype(hsa_signal_xor_acq_rel)* hsa_signal_xor_acq_rel_t;
typedef decltype(hsa_signal_xor_scacquire)* hsa_signal_xor_scacquire_t;
typedef decltype(hsa_signal_xor_acquire)* hsa_signal_xor_acquire_t;
typedef decltype(hsa_signal_xor_relaxed)* hsa_signal_xor_relaxed_t;
typedef decltype(hsa_signal_xor_screlease)* hsa_signal_xor_screlease_t;
typedef decltype(hsa_signal_xor_release)* hsa_signal_xor_release_t;
typedef decltype(hsa_signal_wait_acquire)* hsa_signal_wait_acquire_t;
typedef decltype(hsa_signal_wait_scacquire)* hsa_signal_wait_scacquire_t;
typedef decltype(hsa_signal_wait_relaxed)* hsa_signal_wait_relaxed_t;
typedef decltype(hsa_signal_group_create)* hsa_signal_group_create_t;
typedef decltype(hsa_signal_group_destroy)* hsa_signal_group_destroy_t;
typedef decltype(hsa_signal_group_wait_any_scacquire)* hsa_signal_group_wait_any_scacquire_t;
typedef decltype(hsa_signal_group_wait_any_relaxed)* hsa_signal_group_wait_any_relaxed_t;
typedef decltype(hsa_queue_create)* hsa_queue_create_t;
typedef decltype(hsa_soft_queue_create)* hsa_soft_queue_create_t;
typedef decltype(hsa_queue_destroy)* hsa_queue_destroy_t;
typedef decltype(hsa_queue_inactivate)* hsa_queue_inactivate_t;
typedef decltype(hsa_queue_load_read_index_acquire)* hsa_queue_load_read_index_acquire_t;
typedef decltype(hsa_queue_load_read_index_scacquire)* hsa_queue_load_read_index_scacquire_t;
typedef decltype(hsa_queue_load_read_index_relaxed)* hsa_queue_load_read_index_relaxed_t;
typedef decltype(hsa_queue_load_write_index_acquire)* hsa_queue_load_write_index_acquire_t;
typedef decltype(hsa_queue_load_write_index_scacquire)* hsa_queue_load_write_index_scacquire_t;
typedef decltype(hsa_queue_load_write_index_relaxed)* hsa_queue_load_write_index_relaxed_t;
typedef decltype(hsa_queue_store_write_index_relaxed)* hsa_queue_store_write_index_relaxed_t;
typedef decltype(hsa_queue_store_write_index_release)* hsa_queue_store_write_index_release_t;
typedef decltype(hsa_queue_store_write_index_screlease)* hsa_queue_store_write_index_screlease_t;
typedef decltype(hsa_queue_cas_write_index_acq_rel)* hsa_queue_cas_write_index_acq_rel_t;
typedef decltype(hsa_queue_cas_write_index_scacq_screl)* hsa_queue_cas_write_index_scacq_screl_t;
typedef decltype(hsa_queue_cas_write_index_acquire)* hsa_queue_cas_write_index_acquire_t;
typedef decltype(hsa_queue_cas_write_index_scacquire)* hsa_queue_cas_write_index_scacquire_t;
typedef decltype(hsa_queue_cas_write_index_relaxed)* hsa_queue_cas_write_index_relaxed_t;
typedef decltype(hsa_queue_cas_write_index_release)* hsa_queue_cas_write_index_release_t;
typedef decltype(hsa_queue_cas_write_index_screlease)* hsa_queue_cas_write_index_screlease_t;
typedef decltype(hsa_queue_add_write_index_acq_rel)* hsa_queue_add_write_index_acq_rel_t;
typedef decltype(hsa_queue_add_write_index_scacq_screl)* hsa_queue_add_write_index_scacq_screl_t;
typedef decltype(hsa_queue_add_write_index_acquire)* hsa_queue_add_write_index_acquire_t;
typedef decltype(hsa_queue_add_write_index_scacquire)* hsa_queue_add_write_index_scacquire_t;
typedef decltype(hsa_queue_add_write_index_relaxed)* hsa_queue_add_write_index_relaxed_t;
typedef decltype(hsa_queue_add_write_index_release)* hsa_queue_add_write_index_release_t;
typedef decltype(hsa_queue_add_write_index_screlease)* hsa_queue_add_write_index_screlease_t;
typedef decltype(hsa_queue_store_read_index_relaxed)* hsa_queue_store_read_index_relaxed_t;
typedef decltype(hsa_queue_store_read_index_release)* hsa_queue_store_read_index_release_t;
typedef decltype(hsa_queue_store_read_index_screlease)* hsa_queue_store_read_index_screlease_t;
typedef decltype(hsa_region_get_info)* hsa_region_get_info_t;
typedef decltype(hsa_agent_iterate_regions)* hsa_agent_iterate_regions_t;
typedef decltype(hsa_memory_allocate)* hsa_memory_allocate_t;
typedef decltype(hsa_memory_free)* hsa_memory_free_t;
typedef decltype(hsa_memory_copy)* hsa_memory_copy_t;
typedef decltype(hsa_memory_assign_agent)* hsa_memory_assign_agent_t;
typedef decltype(hsa_memory_register)* hsa_memory_register_t;
typedef decltype(hsa_memory_deregister)* hsa_memory_deregister_t;
typedef decltype(hsa_isa_from_name)* hsa_isa_from_name_t;
typedef decltype(hsa_agent_iterate_isas)* hsa_agent_iterate_isas_t;
typedef decltype(hsa_isa_get_info)* hsa_isa_get_info_t;
typedef decltype(hsa_isa_get_info_alt)* hsa_isa_get_info_alt_t;
typedef decltype(hsa_isa_get_exception_policies)* hsa_isa_get_exception_policies_t;
typedef decltype(hsa_isa_get_round_method)* hsa_isa_get_round_method_t;
typedef decltype(hsa_wavefront_get_info)* hsa_wavefront_get_info_t;
typedef decltype(hsa_isa_iterate_wavefronts)* hsa_isa_iterate_wavefronts_t;
typedef decltype(hsa_isa_compatible)* hsa_isa_compatible_t;
typedef decltype(hsa_code_object_reader_create_from_file)* hsa_code_object_reader_create_from_file_t;
typedef decltype(hsa_code_object_reader_create_from_memory)* hsa_code_object_reader_create_from_memory_t;
typedef decltype(hsa_code_object_reader_destroy)* hsa_code_object_reader_destroy_t;
typedef decltype(hsa_executable_create)* hsa_executable_create_t;
typedef decltype(hsa_executable_create_alt)* hsa_executable_create_alt_t;
typedef decltype(hsa_executable_destroy)* hsa_executable_destroy_t;
typedef decltype(hsa_executable_load_program_code_object)* hsa_executable_load_program_code_object_t;
typedef decltype(hsa_executable_load_agent_code_object)* hsa_executable_load_agent_code_object_t;
typedef decltype(hsa_executable_freeze)* hsa_executable_freeze_t;
typedef decltype(hsa_executable_get_info)* hsa_executable_get_info_t;
typedef decltype(hsa_executable_global_variable_define)* hsa_executable_global_variable_define_t;
typedef decltype(hsa_executable_agent_global_variable_define)* hsa_executable_agent_global_variable_define_t;
typedef decltype(hsa_executable_readonly_variable_define)* hsa_executable_readonly_variable_define_t;
typedef decltype(hsa_executable_validate)* hsa_executable_validate_t;
typedef decltype(hsa_executable_validate_alt)* hsa_executable_validate_alt_t;
typedef decltype(hsa_executable_get_symbol)* hsa_executable_get_symbol_t;
typedef decltype(hsa_executable_get_symbol_by_name)* hsa_executable_get_symbol_by_name_t;
typedef decltype(hsa_executable_symbol_get_info)* hsa_executable_symbol_get_info_t;
typedef decltype(hsa_executable_iterate_symbols)* hsa_executable_iterate_symbols_t;
typedef decltype(hsa_executable_iterate_agent_symbols)* hsa_executable_iterate_agent_symbols_t;
typedef decltype(hsa_executable_iterate_program_symbols)* hsa_executable_iterate_program_symbols_t;
typedef decltype(hsa_code_object_serialize)* hsa_code_object_serialize_t;
typedef decltype(hsa_code_object_deserialize)* hsa_code_object_deserialize_t;
typedef decltype(hsa_code_object_destroy)* hsa_code_object_destroy_t;
typedef decltype(hsa_code_object_get_info)* hsa_code_object_get_info_t;
typedef decltype(hsa_executable_load_code_object)* hsa_executable_load_code_object_t;
typedef decltype(hsa_code_object_get_symbol)* hsa_code_object_get_symbol_t;
typedef decltype(hsa_code_object_get_symbol_from_name)* hsa_code_object_get_symbol_from_name_t;
typedef decltype(hsa_code_symbol_get_info)* hsa_code_symbol_get_info_t;
typedef decltype(hsa_code_object_iterate_symbols)* hsa_code_object_iterate_symbols_t;
typedef decltype(hsa_ext_program_create)* hsa_ext_program_create_t;
typedef decltype(hsa_ext_program_destroy)* hsa_ext_program_destroy_t;
typedef decltype(hsa_ext_program_add_module)* hsa_ext_program_add_module_t;
typedef decltype(hsa_ext_program_iterate_modules)* hsa_ext_program_iterate_modules_t;
typedef decltype(hsa_ext_program_get_info)* hsa_ext_program_get_info_t;
typedef decltype(hsa_ext_program_finalize)* hsa_ext_program_finalize_t;

typedef decltype(hsa_ext_image_get_capability)* hsa_ext_image_get_capability_t;
typedef decltype(hsa_ext_image_data_get_info)* hsa_ext_image_data_get_info_t;
typedef decltype(hsa_ext_image_create)* hsa_ext_image_create_t;
typedef decltype(hsa_ext_image_destroy)* hsa_ext_image_destroy_t;
typedef decltype(hsa_ext_image_copy)* hsa_ext_image_copy_t;
typedef decltype(hsa_ext_image_import)* hsa_ext_image_import_t;
typedef decltype(hsa_ext_image_export)* hsa_ext_image_export_t;
typedef decltype(hsa_ext_image_clear)* hsa_ext_image_clear_t;
typedef decltype(hsa_ext_sampler_create)* hsa_ext_sampler_create_t;
typedef decltype(hsa_ext_sampler_destroy)* hsa_ext_sampler_destroy_t;

typedef decltype(hsa_amd_coherency_get_type)* hsa_amd_coherency_get_type_t;
typedef decltype(hsa_amd_coherency_set_type)* hsa_amd_coherency_set_type_t;
typedef decltype(hsa_amd_profiling_set_profiler_enabled)* hsa_amd_profiling_set_profiler_enabled_t;
typedef decltype(hsa_amd_profiling_async_copy_enable)* hsa_amd_profiling_async_copy_enable_t;
typedef decltype(hsa_amd_profiling_get_dispatch_time)* hsa_amd_profiling_get_dispatch_time_t;
typedef decltype(hsa_amd_profiling_get_async_copy_time)* hsa_amd_profiling_get_async_copy_time_t;
typedef decltype(hsa_amd_profiling_convert_tick_to_system_domain)* hsa_amd_profiling_convert_tick_to_system_domain_t;
typedef decltype(hsa_amd_signal_async_handler)* hsa_amd_signal_async_handler_t;
typedef decltype(hsa_amd_async_function)* hsa_amd_async_function_t;
typedef decltype(hsa_amd_signal_wait_any)* hsa_amd_signal_wait_any_t;
typedef decltype(hsa_amd_image_get_info_max_dim)* hsa_amd_image_get_info_max_dim_t;
typedef decltype(hsa_amd_queue_cu_set_mask)* hsa_amd_queue_cu_set_mask_t;
typedef decltype(hsa_amd_memory_pool_get_info)* hsa_amd_memory_pool_get_info_t;
typedef decltype(hsa_amd_agent_iterate_memory_pools)* hsa_amd_agent_iterate_memory_pools_t;
typedef decltype(hsa_amd_memory_pool_allocate)* hsa_amd_memory_pool_allocate_t;
typedef decltype(hsa_amd_memory_pool_free)* hsa_amd_memory_pool_free_t;
typedef decltype(hsa_amd_memory_async_copy)* hsa_amd_memory_async_copy_t;
typedef decltype(hsa_amd_agent_memory_pool_get_info)* hsa_amd_agent_memory_pool_get_info_t;
typedef decltype(hsa_amd_agents_allow_access)* hsa_amd_agents_allow_access_t;
typedef decltype(hsa_amd_memory_pool_can_migrate)* hsa_amd_memory_pool_can_migrate_t;
typedef decltype(hsa_amd_memory_migrate)* hsa_amd_memory_migrate_t;
typedef decltype(hsa_amd_memory_lock)* hsa_amd_memory_lock_t;
typedef decltype(hsa_amd_memory_unlock)* hsa_amd_memory_unlock_t;
typedef decltype(hsa_amd_memory_fill)* hsa_amd_memory_fill_t;
typedef decltype(hsa_amd_interop_map_buffer)* hsa_amd_interop_map_buffer_t;
typedef decltype(hsa_amd_interop_unmap_buffer)* hsa_amd_interop_unmap_buffer_t;
typedef decltype(hsa_amd_image_create)* hsa_amd_image_create_t;

typedef decltype(hsa_ven_amd_loader_query_host_address)* hsa_ven_amd_loader_query_host_address_t;
typedef decltype(hsa_ven_amd_loader_query_segment_descriptors)* hsa_ven_amd_loader_query_segment_descriptors_t;



#endif // _HSARTMODULEDECLS_H_

